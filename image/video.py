...
class Video():
    ...
    def __init__(self,
        ...
        self.size = (int(self.video.get(cv.CAP_PROP_FRAME_WIDTH)), int(self.video.get(cv.CAP_PROP_FRAME_HEIGHT)))

    def process_video(self):
        fourcc = cv.VideoWriter_fourcc(*'MJPG')
        videoWriter = cv.VideoWriter('results/output.avi', fourcc, self.fps, self.size)
        success, frame = self.video.read()
        videoWriter.write(frame)
        while success:
            self.cur_human_image = frame
            try:
                a = TwoImages(person_input=frame, comic_input=self.comic_image.im)
                im = a.fusion_rotated()
